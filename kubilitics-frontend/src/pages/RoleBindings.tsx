...
import { Link2, Search, RefreshCw, MoreHorizontal, Loader2, WifiOff, Plus, ChevronDown } from 'lucide-react';
...
export default function RoleBindings() {
  ...
  const { data, isLoading, refetch } = usePaginatedResourceList<RoleBindingResource>('rolebindings');
  const deleteResource = useDeleteK8sResource('rolebindings');

  const [deleteDialog, setDeleteDialog] = useState<{ open: boolean; item: RoleBindingRow | null }>({ open: false, item: null });
    ...
    <>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {searchFiltered.length} role bindings
                {!isConnected && (
        ...
        <div className={cn('grid grid-cols-2 sm:grid-cols-4 gap-4', !isConnected && 'opacity-60')}>
          <ListPageStatCard label="Total Role Bindings" value={stats.total} icon={Link2} iconColor="text-primary" />
            ...
            <Table className="table-fixed" style={{ minWidth: 950 }}>
              <TableHeader>
                <TableRow className="bg-muted/40 hover:bg-muted/40 border-b border-border/80">
                  ...
                  <ResizableTableHead columnId="role"><span className="text-sm font-medium">Role</span></ResizableTableHead>
                  <ResizableTableHead columnId="roleKind"><span className="text-sm font-medium">Role Kind</span></ResizableTableHead>
                  ...
                  itemsOnPage.map((r, idx) => (
                    <motion.tr key={`${r.namespace}/${r.name}`} initial={ROW_MOTION.initial} animate={ROW_MOTION.animate} transition={ROW_MOTION.transition(idx)} className={cn(resourceTableRowClassName, idx % 2 === 1 && 'bg-muted/5')}>
                      ...
                      <ResizableTableCell columnId="role">
                        <Link to={roleLink(r)} className="text-primary hover:underline font-mono text-sm truncate block">{r.roleName}</Link>
                      </ResizableTableCell>
